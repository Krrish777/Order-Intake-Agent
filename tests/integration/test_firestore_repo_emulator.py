...
from backend.data import FirestoreRepo, get_async_client
...
@pytest.fixture
async def repo() -> FirestoreRepo:
    r = FirestoreRepo(get_async_client())
    try:
        yield r
    finally:
        await r.aclose()
...
async def test_get_product_hit(repo: FirestoreRepo) -> None:
...
async def test_get_product_miss(repo: FirestoreRepo) -> None:
    assert await repo.get_product("NOPE-DOES-NOT-EXIST") is None


async def test_get_products_batch(repo: FirestoreRepo) -> None:
...
async def test_list_all_products_has_catalog(repo: FirestoreRepo) -> None:
...
async def test_get_customer_hit(repo: FirestoreRepo) -> None:
...
async def test_get_customer_miss(repo: FirestoreRepo) -> None:
    assert await repo.get_customer("CUST-99999") is None


async def test_find_customer_by_name(repo: FirestoreRepo) -> None:
    c = await repo.find_customer_by_name("Patterson Industrial")
    assert c is not None
    assert c.customer_id == "CUST-00042"


async def test_find_customer_by_name_below_threshold(repo: FirestoreRepo) -> None:
...
async def test_get_meta(repo: FirestoreRepo) -> None:
    m = await repo.get_meta()
    assert isinstance(m, MetaRecord)
    assert m.catalog_version.startswith("2026-Q2")


async def test_find_product_by_embedding_returns_empty_stub(repo: FirestoreRepo) -> None:
