...
date: 2026-04-20
last_updated: 2026-04-22 (Track A — Steps 5 + 6 + 6.5 landed. 5858608 added the first end-to-end integration test via ADK `Runner` against the Firestore emulator (stubs clarify + summary LlmAgents with `FakeChildLlmAgent`, everything else real), narrowed the Step-5 conftest env-setdefault to `tests/unit/conftest.py` so integration tests retain their explicit skip-or-fail semantics, and added a client-lifetime comment in `_build_default_root_agent` documenting the process-scoped ownership. That test surfaced a pre-existing parser bug — `document_parser/legacy/parser.py` passed raw filenames to LlamaCloud's `external_file_id` which is unique-per-workspace, so re-runs on the same fixture collided with `UniqueViolationError`. 3151c24 (Step 6.5) ported the classifier's hash-suffix shape into the parser (`f"{filename}::{sha256(content)[:12]}"` — content-hashed for idempotent re-upload vs classifier's UUID form) with 2 new deterministic-vs-unique tests, and folded the Step-6 code-review follow-ups: integration test now asserts against FinalizeStage's deterministic pre-seeds (`state["orders_created"]/["exceptions_opened"]/["docs_skipped"]`) in addition to the stubbed `run_summary` response, and the "Runner survivability" claim in the test docstring is softened to what the AUTO_APPROVE path actually demonstrates. Also: FakeChildLlmAgent proven to survive `SequentialAgent.model_copy` through the real Runner (the Step 4f code-review concern). Unit suite at 297 passed (295 → 297). Only Step 7 (smoke evalset via adk eval) + Step 8 (adk web manual smoke) + Step 9 (close-out doc refresh) remain on Track A.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A — pipeline end-to-end proven ✓**: Steps 1-3 ✓ · All 8 BaseAgent stages ✓ (4a-4h) · **Step 5 ✓** root SequentialAgent assembled · **Step 6 ✓** end-to-end integration test via ADK Runner against the Firestore emulator (AUTO_APPROVE path patterson_po-28491.wrapper.eml → OrderRecord lands, deterministic counts verified, FakeChildLlmAgent proven to survive SequentialAgent.model_copy) · **Step 6.5 ✓** parser external_file_id hash-suffix fix (unblocks Step 7 evalset re-runs). Also: bytes round-trip fix ✓ · Test-helper pull-up ✓ · `FakeChildLlmAgent` shared fake ✓ · PersistStage body-dict assertion ✓ · Unit-scoped conftest for emulator-env default ✓. | **Step 7 is next**: `tests/eval/smoke.evalset.json` with 4 cases + `eval_config.json`; run via `adk eval backend/my_agent tests/eval/smoke.evalset.json` with `GOOGLE_API_KEY` set (first LLM-live gate on the whole pipeline). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
