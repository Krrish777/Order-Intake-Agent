...
date: 2026-04-20
last_updated: 2026-04-22 (Track A — Step 5 landed: root `SequentialAgent` assembled. Commit 847d6eb rewrites `backend/my_agent/agent.py` with (a) pure kwarg-only factory `build_root_agent(*, classify_fn, parse_fn, validator, coordinator, clarify_agent, summary_agent, exception_store)` returning `SequentialAgent('order_intake_pipeline', sub_agents=[8 stages in canonical order])`, (b) `_build_default_root_agent()` that constructs real deps (shared async Firestore client → MasterDataRepo/OrderValidator/FirestoreOrderStore/FirestoreExceptionStore/IntakeCoordinator(agent_version='track-a-v0.1')/2 LlmAgents/raw classify_document+parse_document callables), (c) module-level `root_agent` for adk web discovery. Canonical order is load-bearing for Steps 6-8: ingest→reply_shortcircuit→classify→parse→validate→clarify→persist→finalize. Topology test asserts name, ordered sub_agents by imported `*_STAGE_NAME` constants, subclass types, kwarg-only enforcement, distinct-instance-per-call, missing-dep TypeError — 6 tests. `tests/conftest.py` added (32 lines, `os.environ.setdefault` for FIRESTORE_EMULATOR_HOST + GOOGLE_CLOUD_PROJECT) so test collection doesn't trip on the new import-time Firestore client construction; `tests/conftest.py`'s scope affects integration tests too (tracked follow-up for Step 6 to resolve). FinalizeStage docstring also gained a note that `reply_handled` is authoritatively re-written on session state when FinalizeStage seeds the summary template (4h code-review follow-up). +6 new unit tests; suite at 295 passed. Only Step 6 (emulator integration) + Step 7 (smoke evalset via adk eval) + Step 8 (adk web manual smoke) + Step 9 (close-out doc refresh) remain on Track A.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A — pipeline now assembled ✓**: Steps 1-3 ✓ · All 8 BaseAgent stages landed ✓ (4a IngestStage · 4b ReplyShortCircuitStage · 4c ClassifyStage · 4d ParseStage · 4e ValidateStage · 4f ClarifyStage · 4g PersistStage · 4h FinalizeStage) · **Step 5 ✓** `backend/my_agent/agent.py` rewrite: `build_root_agent(*, 7-kwarg-only deps) → SequentialAgent('order_intake_pipeline', sub_agents=[8 stages in canonical order])` + `_build_default_root_agent()` with shared Firestore client + module-level `root_agent` for adk web discovery. Also: bytes round-trip fix ✓ · Test-helper pull-up ✓ · `FakeChildLlmAgent` shared fake ✓ · PersistStage body-dict assertion ✓ · tests/conftest.py emulator-env default ✓. | **Step 6 is next**: end-to-end integration test against the Firestore emulator (stubbed clarify + summary LlmAgents; real validator + real coordinator; drives patterson_po-28491.wrapper.eml through the assembled `root_agent` via `Runner.run_async`; asserts OrderRecord lands in emulator). Step 7 smoke evalset (4 cases). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
...
- `build_root_agent`, `_build_default_root_agent`, `root_agent`, `ROOT_AGENT_NAME`, `AGENT_VERSION` → `backend/my_agent/agent.py` **✓ on master (847d6eb, 2026-04-22)** — pure kwarg-only factory + module-level assembled `SequentialAgent` for adk web discovery. Step 6's integration test and Step 7's evalset both bind to this surface.
