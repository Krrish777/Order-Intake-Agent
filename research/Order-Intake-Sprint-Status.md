...
date: 2026-04-20
last_updated: 2026-04-22 (Track A in flight — 11 commits on master: contracts / output-schema models / LlmAgent factories / IngestStage / ReplyShortCircuitStage / ClassifyStage / bytes round-trip fix / ParseStage / ValidateStage / ClarifyStage (BaseAgent #6/8 — first stage to hold a child LlmAgent; iterates CLARIFY-tier validation_results, directly mutates `ctx.session.state["customer_name" | "original_subject" | "reason"]` before invoking `self._clarify_agent.run_async(ctx)` — necessary escape from state_delta discipline since LlmAgent interpolates its instruction template against live session.state and no Runner-loop applies deltas between parent-and-child within one invocation; captures `clarify_email` from the last event carrying it in state_delta; keys `clarify_bodies` by `"{filename}#{sub_doc_index}"`; child typed as `Any` via PrivateAttr so plain duck-typed fakes work without Pydantic-isinstance trouble) / PersistStage (BaseAgent #7/8 — awaits `IntakeCoordinator.process(parsed, envelope, order_index=sub_doc_index, clarify_body=...)` per parsed_docs entry, pulling the body STRING from `clarify_bodies[key]["body"]` when present; accumulates `ProcessResult.model_dump(mode="json")` entries keyed back by filename+sub_doc_index). +56 new unit tests (7 validate + 8 clarify + 1 merge-semantic + 9 persist landed this wave); suite at 281 passed. ADK/test findings accumulated: (a) PrivateAttr for all injected deps (Protocol, concrete, Callable, or LlmAgent-as-Any — uniform); (b) `asyncio.to_thread` for sync tools, direct `await` for async tools, `async for` + event forwarding for child agents; (c) Pydantic `Base64Bytes` rejects raw binary — symmetric round-trip needs a custom mode="before" validator; (d) `skipped_docs` cumulative audit trail via `list(state.get("skipped_docs", []))`; (e) parent→child state seeding within one invocation requires direct `ctx.session.state[...] = ...` mutation — state_delta doesn't propagate without a Runner loop.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A in flight on master** (no worktree — per-plan decision): Steps 1-3 ✓ · 4a `IngestStage` ✓ · 4b `ReplyShortCircuitStage` ✓ · 4c `ClassifyStage` ✓ · 4d `ParseStage` ✓ · 4e `ValidateStage` ✓ · 4f `ClarifyStage` ✓ (BaseAgent #6: first child-LlmAgent stage; per-CLARIFY-tier entry, direct-mutate prompt-template state keys then `async for event in self._clarify_agent.run_async(ctx): yield event`, capture `clarify_email` from final event's state_delta; `clarify_bodies` dict keyed by `"{filename}#{sub_doc_index}"`) · 4g `PersistStage` ✓ (BaseAgent #7: awaits `IntakeCoordinator.process(parsed, envelope, order_index=..., clarify_body=body_str)`; `process_results` list preserves per-entry routing outcome) · Bytes round-trip fix ✓ · Test-helper pull-up ✓. Stub `backend/my_agent/agent.py` still present ⚠ (Step 5 replaces). | Step 4h `FinalizeStage` (last BaseAgent — summary LlmAgent wraps a RunSummary schema; reuses the child-invoke-and-capture pattern ClarifyStage established). Step 5 assemble `SequentialAgent`. Step 6 emulator integration test. Step 7 smoke evalset (4 cases). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
...
tests/unit/test_stage_persist.py                                        ✓ 9 tests: reply_handled no-op / missing envelope → ValueError / missing parsed_docs → ValueError / empty parsed_docs → empty results / single AUTO_APPROVE (asserts clarify_body=None) / single CLARIFY path (asserts coordinator received the STRING body not the dict) / single ESCALATE (asserts clarify_body=None) / multi-entry ordering preserved / coordinator raising propagates. Fake returns use real Pydantic ProcessResult + OrderRecord + ExceptionRecord instances (6f75572, Track A Step 4g)
```
...
tests/unit/test_stage_finalize.py                         → Track A Step 4h
...
- `PersistStage`, `PERSIST_STAGE_NAME` → `backend/my_agent/stages/persist.py` **✓ on master (6f75572, 2026-04-22)** — wires `IntakeCoordinator.process(..., clarify_body=...)` per parsed sub-doc; `process_results` shape is what Step 5's FinalizeStage will summarize.
