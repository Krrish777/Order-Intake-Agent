...
date: 2026-04-20
last_updated: 2026-04-22 (Track A in flight — 7 commits on master: (1) contracts schema bump; (2) ClarifyEmail + RunSummary models + prompt templates; (3) LlmAgent factories (gemini-3-flash-preview); (4) IngestStage; (5) ReplyShortCircuitStage; (6) ClassifyStage (BaseAgent #3/8, per-attachment classify with asyncio.to_thread around the sync LlamaClassify poll, splits POs into `classified_docs` vs non-POs into `skipped_docs`); (7) EmailAttachment bytes round-trip fix — paired `@field_validator("content", mode="before")` that base64-decodes when input is a base64 string, unblocking ParseStage which will pass `attachment.content` straight to LlamaExtract. +28 new unit tests; suite at 247 passed. ADK findings: (a) PrivateAttr is the dep-injection pattern for Protocol-typed services on BaseAgent; (b) `asyncio.to_thread` cleanly wraps sync 60s-poll tools without blocking the event loop; (c) Pydantic `Base64Bytes` rejects raw binary at construction, so symmetric round-trip needs a custom mode="before" validator.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A in flight on master** (no worktree — per-plan decision): Steps 1-3 ✓ (contracts, models, prompts, LlmAgent factories) · Step 4a `IngestStage` ✓ · Step 4b `ReplyShortCircuitStage` ✓ · Step 4c `ClassifyStage` ✓ (BaseAgent #3: per-attachment classify via `asyncio.to_thread(classify_fn, ...)`, splits `classified_docs` (PO-only) vs `skipped_docs` on state) · Bytes round-trip fix ✓ (`EmailAttachment.content` now symmetric through `model_dump(mode="json")` → `model_validate`). Stub `backend/my_agent/agent.py` still present ⚠ (Step 5 replaces). | Steps 4d-4h: ParseStage, ValidateStage, ClarifyStage, PersistStage, FinalizeStage (5 remaining BaseAgent stages — all reuse PrivateAttr dep-injection + sequential-by-design patterns). **Contract refinement for 4d+:** `skipped_docs` must be APPEND-not-overwrite (read-modify-write via `state.get("skipped_docs", [])` + `.append(...)`) so Classify-time skips survive when Parse/Validate add their own. Step 5 assemble `SequentialAgent`. Step 6 emulator integration test. Step 7 smoke evalset (4 cases). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
...
backend/my_agent/stages/parse.py                  → Track A Step 4d (per-classified-doc LlamaExtract; flattens parsed.sub_documents into (filename, sub_doc_index, parsed, sub_doc) tuples; bytes round-trip unblocked by fda48a0)
...
tests/unit/test_stage_{parse,validate,clarify,persist,finalize}.py  → Track A Steps 4d-4h
...
- `ClassifyStage`, `CLASSIFY_STAGE_NAME`, `ClassifyFn` type alias → `backend/my_agent/stages/classify.py` **✓ on master (18ce553, 2026-04-22)** — establishes the `asyncio.to_thread` pattern for sync blocking tools + the `skipped_docs` contract (must be append-not-overwrite in Steps 4d-4h).
