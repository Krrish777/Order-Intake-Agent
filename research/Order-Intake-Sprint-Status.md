...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A COMPLETE ✓ + live-run verified**: pipeline + all 8 stages + root SequentialAgent + integration test + smoke evalset + operator READMEs all landed. **Live-run audit (23e5812)** fixed 7 findings surfaced by the first real-API smoke: F1 adk_apps/order_intake/ single-entry scan dir, F3 Gemini-schema-safe regression walker (extra="forbid" blocks response_schema), F4 parser UUID external_file_id (was deterministic; LlamaCloud rejects duplicates), F5 precomputed_validation kwarg (halves validator cost — ValidateStage + PersistStage no longer double-run), F14 evalset schema (user_content.role removed, app_name="order_intake"), F16 consolidated .env. 304 unit tests + 10+ integration + 3-case smoke evalset. End-to-end smoke against real Gemini+LlamaCloud+emulator captured at research/Track-A-Live-Smoke-2026-04-22.log. | Nothing on Track A. Work shifts to Tracks D / E / Demo + deferred items (Gmail push ingestion, embedding Tier 3, Learning Loop, auto-merge reply path, CLARIFY/ESCALATE evalset cases, dashboard deploy) + audit follow-ups (F6 line-item matcher tuning, F8/F19 before_model_callback polish). |
...
**Read + judgment + persist + orchestrate are complete.** The entire `.eml → EmailEnvelope → (ClassifiedDocument) → ParsedDocument → ValidationResult → (ClarifyEmail) → ProcessResult → RunSummary` pipeline runs end-to-end through the assembled `SequentialAgent('order_intake_pipeline')` at `backend/my_agent/agent.py`. `adk web .` discovers it at import time; `adk eval backend/my_agent tests/eval/smoke.evalset.json --config_file_path tests/eval/eval_config.json` exercises 3 live cases. The Step 6 integration test drives the AUTO_APPROVE path through `Runner.run_async` against the Firestore emulator deterministically (stubbed LlmAgents).

**What's left is the surface half** — Track D (read-only dashboard), Track E (expanded evalsets + CLARIFY/ESCALATE cases post-live-discovery), Track Demo (`scripts/run_demo.py`), and the Post-MVP items roadmapped in `Glacis-Order-Intake.md` (Gmail push, embedding Tier 3, Learning Loop, auto-merge reply, quality-gate judge, cloud deploy). No in-sprint blockers remain.

**Completion metrics:** 297 unit tests + 10+ integration tests + 3 smoke evalset cases. 8 BaseAgent stages + 2 Gemini LlmAgents + 1 SequentialAgent + 6 persistence/validator primitives + 1 end-to-end test harness + 1 operator README. ~98% of MVP-scoped code lines done; remaining 2% is dashboard + demo polish.
...
backend/my_agent/README.md                                              ✓ `adk web` operator guide (207 lines): one-line "what is this" + 4 prereqs (uv sync / emulator / master data / creds) + launch command (`uv run adk web .`) + what to type in the UI (fixture paths or raw EML) + 8-stage event-stream table + expected final state + 3 recommended fixtures (patterson AUTO, birch_valley reply, redline trace-interesting) + known quirks + 4-item troubleshooting + cross-links to tests/eval/README.md and sprint status (59d4f84, Track A Step 8)
```
...
Track A closed. Priority order for the remaining work:

1. **`feat/dashboard` (Track D)** — Firebase Hosting SPA reading from the `orders` + `exceptions` Firestore collections that Track A now populates. Read-only for MVP: order list with status badges + exception detail view showing `parsed_doc.sub_documents` + `validation_result.lines[].notes` + (for CLARIFY cases) the `clarify_body` the ClarifyStage generated. Approve/reject deferred.
2. **`feat/demo-script` (Track Demo)** — `scripts/run_demo.py` that drives 3 fixtures through the assembled agent, writes a JSON summary of each run, and prints a 2-minute-video-worthy trace.
3. **`feat/eval` (Track E)** — expand `tests/eval/smoke.evalset.json` to the full 3-scenario golden set Glacis's demo plan calls for; tighten thresholds; add CLARIFY + ESCALATE cases (deferred from Step 7 pending a live validator-discovery run to pick fixtures in the right confidence band).
4. **First live `adk eval` + `adk web` run** — requires `GOOGLE_API_KEY` + `LLAMA_CLOUD_API_KEY` + running emulator + seeded master data. The operator READMEs at `backend/my_agent/README.md` and `tests/eval/README.md` are the runbooks. Outcome feeds back into Track E (threshold tuning) + any bug fixes surfacing in prompt drift.
...
- `tests/integration/test_orchestrator_emulator.py` end-to-end pattern **✓ on master (5858608 + 3151c24, 2026-04-22)** — the reference shape for driving the full 8-stage pipeline through `Runner.run_async` with stubbed LlmAgents + real validator/coordinator/stores. Step 7's evalset harness + any post-sprint CLARIFY/ESCALATE/reply-path integration tests copy from here.
...
- Gemini-schema-safe regression walker (tests/unit/test_llm_agent_factories.py) **✓ on master (23e5812, 2026-04-22)** — asserts no LlmAgent's output_schema emits `additionalProperties: false` (Gemini's `generation_config.response_schema` rejects that field with a 400; Pydantic emits it for any model marked `extra="forbid"`).
