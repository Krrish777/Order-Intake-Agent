...
date: 2026-04-20
last_updated: 2026-04-22 (**Track A COMPLETE ✓** — 17 commits on master across Steps 1-9 under subagent-driven development, spec + code-quality reviewed each. Final deliverables: all 8 BaseAgent stages (Ingest / ReplyShortCircuit / Classify / Parse / Validate / Clarify / Persist / Finalize) wired into `SequentialAgent('order_intake_pipeline')` at `backend/my_agent/agent.py`, module-level `root_agent` for `adk web` discovery, pure kwarg-only `build_root_agent(...)` factory for tests, end-to-end integration test against the Firestore emulator (AUTO_APPROVE path), smoke evalset with 3 cases + seed helper + operator README at `backend/my_agent/README.md`. 297 unit tests + 10+ integration tests passing. Parser bug fix (3151c24) mirrors the classifier's hash-suffix pattern so re-runs don't collide on LlamaCloud's unique-per-workspace `external_file_id`. Sprint scope closes out with Tracks D (dashboard), E (eval expansion to full 3-scenario golden set), Demo (scripts/run_demo.py) still open. Follow-up items tracked in-file: CLARIFY/ESCALATE evalset cases deferred pending live validator discovery; app_name verification deferred to first live `adk eval` run; one minor polish item (FinalizeStage reply_handled write-back semantics) documented as intentional.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A COMPLETE ✓**: pipeline + all 8 stages + root SequentialAgent + integration test + smoke evalset + operator README all landed on master. 297 unit tests + 10+ integration + `adk eval` smoke set (3 cases, 2 deferred w/ reason). Surface is stable for Track D (dashboard) + Track E (eval expansion) + Track Demo to consume. | Nothing on Track A — work shifts to Tracks D / E / Demo + deferred items (Gmail push ingestion, embedding Tier 3, Learning Loop, auto-merge reply path, CLARIFY/ESCALATE evalset cases, dashboard deploy, real-Gemini eval run). |
...
- Parser `external_file_id` hash-suffix `f"{filename}::{sha256(content)[:12]}"` → `backend/tools/document_parser/legacy/parser.py` **✓ on master (3151c24, 2026-04-22)** — idempotent re-upload into LlamaCloud; unblocks Step 7's evalset runs which would otherwise conflict on the second pass.
