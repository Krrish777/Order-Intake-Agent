...
date: 2026-04-20
last_updated: 2026-04-22 (Track A — all 8 BaseAgent stages landed on master: IngestStage, ReplyShortCircuitStage, ClassifyStage, ParseStage, ValidateStage, ClarifyStage, PersistStage, FinalizeStage. Step 4h (6eed197) added BaseAgent #8 — `FinalizeStage` always runs (no reply_handled short-circuit); computes orders_created/exceptions_opened/docs_skipped/reply_handled deterministic counts from upstream state, seeds them on `ctx.session.state` for the summary LlmAgent's `{placeholder}` template, invokes `summary_agent.run_async(ctx)` exactly once, captures `RunSummary` from final event's state_delta. Same commit promoted the duck-typed child-agent fake into `tests/unit/_stage_testing.py` as `FakeChildLlmAgent` (parameterized on output_key + capture_keys + responses; handles three modes: None → RuntimeError path, empty list → stub, populated list → pop-and-emit); ClarifyStage tests refactored to use it. Same commit hardened PersistStage's body-dict lookup with `assert "body" in body_dict` (fail-fast on ClarifyStage schema drift) + added a malformed-body test. Also landed the Glacis roadmap refresh (6a5ee04) flipping §9 Clarify items ⚠ → ✓ and §14 Firestore emulator tag ⚠ → ✓. +8 new unit tests (7 finalize + 1 malformed-body in persist; clarify refactor swapped fakes with no test-count change); suite at 289 passed. Only Step 5 (assemble root SequentialAgent) + Step 6 (emulator integration) + Step 7 (smoke evalset via adk eval) + Step 8 (adk web manual smoke) + Step 9 (close-out doc refresh) remain on Track A.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A in flight on master** (no worktree — per-plan decision): Steps 1-3 ✓ · **All 8 BaseAgent stages landed ✓**: 4a `IngestStage` · 4b `ReplyShortCircuitStage` · 4c `ClassifyStage` · 4d `ParseStage` · 4e `ValidateStage` · 4f `ClarifyStage` · 4g `PersistStage` · 4h `FinalizeStage` (always runs; seeds orders_created/exceptions_opened/docs_skipped/reply_handled on state for the summary LlmAgent; `async for event in summary_agent.run_async(ctx)` exactly once; captures `run_summary` from final event's state_delta). Bytes round-trip fix ✓ · Test-helper pull-up ✓ · `FakeChildLlmAgent` shared duck-typed fake in `_stage_testing.py` ✓ · PersistStage body-dict assertion (fail-fast on ClarifyStage schema drift) ✓. Stub `backend/my_agent/agent.py` still present ⚠ (Step 5 replaces). | **Step 5 is next**: `build_root_agent()` factory that wires the 8 stages into `SequentialAgent`; topology test asserts stage order + names. Step 6 emulator integration test. Step 7 smoke evalset (4 cases). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
