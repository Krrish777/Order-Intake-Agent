...
date: 2026-04-20
last_updated: 2026-04-22 (Track A in flight — 9 commits on master: contracts schema bump / output-schema models / LlmAgent factories / IngestStage / ReplyShortCircuitStage / ClassifyStage / EmailAttachment bytes round-trip fix / ParseStage / ValidateStage (BaseAgent #5/8 — awaits `OrderValidator.validate(order)` per flattened parsed_docs entry, writes `validation_results` keyed by (filename, sub_doc_index) with full `ValidationResult.model_dump(mode="json")` preserving the RoutingDecision enum; pass-through on skipped_docs since CLARIFY/ESCALATE are business outcomes, not pipeline skips; direct `await` — validator is native async, no `asyncio.to_thread`). Alongside 4e, pulled up the duplicated `_make_ctx` helper into `tests/unit/_stage_testing.py` (exports `make_stage_ctx`, `collect_events`, `final_state_delta`) and refactored the 4 existing stage test files to use it (~150 lines deleted net). +44 new unit tests (37 baseline + 7 validate); suite at 263 passed. ADK/test findings: (a) PrivateAttr is the dep-injection pattern for Protocol-typed services AND kept uniform for concrete-class deps (template consistency > marginal benefit of Pattern A); (b) `asyncio.to_thread` for sync tools, direct `await` for async tools; (c) Pydantic `Base64Bytes` rejects raw binary — symmetric round-trip needs a custom mode="before" validator; (d) `skipped_docs` = cumulative audit trail — every stage does `list(state.get("skipped_docs", []))` defensive-copy + extend.)
tags:
  - sprint
  - status
  - gap-analysis
...
| **Orchestration** | ADK SequentialAgent wiring stages | **Track A in flight on master** (no worktree — per-plan decision): Steps 1-3 ✓ · Step 4a `IngestStage` ✓ · Step 4b `ReplyShortCircuitStage` ✓ · Step 4c `ClassifyStage` ✓ · Step 4d `ParseStage` ✓ · Step 4e `ValidateStage` ✓ (BaseAgent #5: re-hydrates each `entry["sub_doc"]` via `ExtractedOrder.model_validate`, direct `await self._validator.validate(order)` per entry — no `asyncio.to_thread` since OrderValidator is native async; writes `validation_results` list as `[{filename, sub_doc_index, validation: model_dump(mode="json")}]` preserving RoutingDecision enum; no additions to skipped_docs) · Bytes round-trip fix ✓ · Test-helper pull-up ✓ (`tests/unit/_stage_testing.py` now owns `make_stage_ctx`/`collect_events`/`final_state_delta`; 4 existing test files refactored). Stub `backend/my_agent/agent.py` still present ⚠ (Step 5 replaces). | Steps 4f-4h: ClarifyStage, PersistStage, FinalizeStage (3 remaining BaseAgent stages). 4f/4h each hold a child LlmAgent (clarify / summary) and invoke via `await child.run_async(ctx)` in a loop; 4g awaits `IntakeCoordinator.process(..., clarify_body=...)`. Step 5 assemble `SequentialAgent`. Step 6 emulator integration test. Step 7 smoke evalset (4 cases). Step 8 `adk web` manual smoke. Step 9 close-out status update. |
...
tests/unit/test_stage_{clarify,persist,finalize}.py  → Track A Steps 4f-4h
...
- `ValidateStage`, `VALIDATE_STAGE_NAME` → `backend/my_agent/stages/validate.py` **✓ on master (1ad3bd2, 2026-04-22)** — establishes direct-`await` pattern for native-async injected deps (contrast: `asyncio.to_thread` for sync tools). Emits `validation_results` shape that ClarifyStage (4f) filters on `validation["decision"] == "clarify"` and PersistStage (4g) routes on the full dict.
- `make_stage_ctx`, `collect_events`, `final_state_delta` → `tests/unit/_stage_testing.py` **✓ on master (1ad3bd2, 2026-04-22)** — shared test helper for stages 4f-4h + eventual Step 5 topology test.
