...
date: 2026-04-21
last_updated: 2026-04-22 (Track P write side landed: §8 ERP Write items flipped MVP ⚠ → MVP ✓; §9 clarify-correlation store side noted as built; §12 Firestore-backed ADK sessions reclassified to Post-MVP per Memory-as-a-Service deferral)
tags:
  - roadmap
  - gap-analysis
  - post-hackathon
...
- `[MVP ⚠]` **Clarify email generator (Track A)** — part of agent orchestration sprint. Must produce a single targeted question, professional tone, same-thread reply. Source: `Exception-Handling.md`, `Prompt-Templates.md`.
- `[MVP ⚠]` **Clarify-reply correlation via `thread_id`** — `ExceptionStore.find_pending_clarify(thread_id)` locates the original exception; `update_with_reply()` advances `PENDING_CLARIFY → AWAITING_REVIEW` with status guard. MVP: store side ✓ landed in Track P (b63b94d, integration-tested against emulator); generator + invocation from agent loop still TBD in Track A. Source: `Exception-Handling.md`.
...
- `[MVP ⚠]` **ADK `SequentialAgent` replacing the stub** — current `backend/my_agent/agent.py` is a stub; Track A replaces it. MVP: planned. Source: `ADK-Order-Intake.md`.
- `[MVP ⚠]` **Stage-per-subagent wiring** — classify → parse → validate → route. Each stage is an independently testable subagent with typed inputs/outputs. MVP: planned in Track A. Source: `ADK-Order-Intake.md`.
...
- `[MVP ⚠]` **Firestore emulator setup** — local-first development. MVP: ✓ via `firebase.json` / `.firebaserc` on master. Source: `Deployment.md`.
