...
- `[MVP ✓]` **Tier 3 — embedding similarity search with `text-embedding-004`** — real `MasterDataRepo.find_product_by_embedding` implementation (Track E, landed 2026-04-25): async `google.genai.Client().aio.models.embed_content(model="text-embedding-004", task_type="RETRIEVAL_QUERY", output_dimensionality=768)` → `AsyncClient.collection("products").find_nearest(vector_field="description_embedding", query_vector=Vector(query_vec), distance_measure=COSINE, limit=k, distance_result_field="__distance")` → cosine-distance-to-similarity (`similarity = 1 − d/2`, clamped) → sorted `EmbeddingMatch` list. `scripts/load_master_data.py` extended to batch-compute + persist `description_embedding: Vector(768)` on each of the 35 products at seed time (RETRIEVAL_DOCUMENT task_type); `--no-embeddings` CLI flag for offline seed runs. `google-genai` already transitive via `google-adk`; zero new pyproject entries. Fail-open on embedding API errors (log + `[]`). Flat `EMBEDDING_THRESHOLD=0.70` in `sku_matcher` kept (rejects Glacis's tiered 0.90/0.70 — validator aggregate handles confidence triage). `backend/my_agent/README.md` documents the `gcloud firestore indexes composite create` CLI for live-Firestore vector index setup; emulator auto-handles find_nearest. MVP: `scripts/load_master_data.py` (cc5c4d5 + 094637f) + `backend/tools/order_validator/tools/master_data_repo.py` (d391e85 + 384f53b) + `backend/my_agent/agent.py` shared genai client wiring (966026b) + 12 new unit tests + 1 gated emulator integration. Source: `Item-Matching.md`, `Firestore-Schema.md`.
- `[MVP ✓]` **Asymmetric embedding task types** — `RETRIEVAL_DOCUMENT` for catalog ingest (`scripts/load_master_data.py:_embed_text`), `RETRIEVAL_QUERY` for customer description (`MasterDataRepo._embed_query`). Free accuracy bump. Both task types pinned via `EmbedContentConfig.task_type`. Source: `Item-Matching.md`.
...
- Gmail push ingestion: `watch()`, Pub/Sub webhook, History API sync, thread tracking (§1) — Poll + Pub/Sub PULL already MVP; PUSH + Cloud Run webhook is the Phase 3 upgrade
- Alias learning from corrections (§5 — embedding portion landed MVP via Track E 2026-04-25; alias-feedback loop remains) + Learning Loop captures (§11)
...
Generator-Judge outbound-email quality gate (§9) landed as MVP (Track B, 2026-04-25). The full 3-stage Generator-Judge validation-loop (§4 — secondary validation before auto-execute, not just outbound review) remains `[Nice-to-have]`.
