...
date: 2026-04-21
last_updated: 2026-04-22 (Track A mid-flight: 7 of 8 BaseAgent stages landed on master under subagent-driven development (IngestStage / ReplyShortCircuitStage / ClassifyStage / ParseStage / ValidateStage / ClarifyStage / PersistStage); §9 Clarify email generator + clarify-reply correlation flipped ⚠ → ✓ (generator is structured-output Gemini LlmAgent; Gmail-send still Post-MVP); §12 ADK orchestration text refreshed to reflect stages-landed progress; §14 Firestore emulator setup flipped ⚠ → ✓ (tag was inconsistent with body text). FinalizeStage + root SequentialAgent assembly still in flight.)
tags:
  - roadmap
  - gap-analysis
  - post-hackathon
...
- `[MVP ⚠]` **ADK `SequentialAgent` replacing the stub** — current `backend/my_agent/agent.py` is a stub; Track A Step 5 will wire `build_root_agent()` that assembles the eight stages. Still in flight pending FinalizeStage landing. Source: `ADK-Order-Intake.md`.
- `[MVP ⚠]` **Stage-per-subagent wiring** — 7 of 8 BaseAgent stages landed on master, each an independently testable subagent with typed inputs/outputs, `PrivateAttr`-based dep injection, and per-stage unit tests (average 7-9 tests each): IngestStage (63780e9), ReplyShortCircuitStage (2566376), ClassifyStage (18ce553), ParseStage (6ba10e8), ValidateStage (1ad3bd2), ClarifyStage (b33a030, first child-LlmAgent stage), PersistStage (6f75572). FinalizeStage (summary LlmAgent) is the eighth and last. Shared test helper at `tests/unit/_stage_testing.py` (`make_stage_ctx`/`collect_events`/`final_state_delta`/`FakeChildLlmAgent`) supports the pattern. Post-Step-5 flip to ✓. Source: `ADK-Order-Intake.md`.
...
- `[MVP ⚠]` **`adk eval` + 3 golden evalsets (Track E)** — snapshot of `ValidationResult` for 3 demo scenarios against seeded fake. MVP: planned. Source: `Overview.md` (eval), `ADK-Order-Intake.md`.
